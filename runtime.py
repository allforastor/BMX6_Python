    ...
    for x, y in psutil.net_if_addrs().items():
        ...
        for z in y:
            ...
            if(z.family is socket.AF_INET):
                ...
                interface.ipv6 = z. address.split("%")[0]
            else:
...
def packet_received(self, ip6):
    ...
    local_list[local_index].pkt_received(self.header)
...
while True:
	...
	packet_header = bmx.packet_header(16,0,0,2,fhandler.link_sqn,pktSqn,main_local.local_id,1)
	...
	bmx.send('ff02::2', 6240, 1, packet)
	print("SENT PACKET:")
	print(bmx.dissect_packet(packet))
	...
	recvdpacket, senderipv6 = bmx.listen('ff02::2', 6240)
