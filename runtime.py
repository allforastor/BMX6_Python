...
class frame_handler:
    ...
    dev_sqn: int = 2                                            # add 1 every send of DEV_ADV
    ...
    def iterate(self):
        for frame in self.frames2send:    
            if(type(frame) == frames.HELLO_ADV):    # HELLO_ADV sqn
                self.hello_sqn = self.hello_sqn + 1
            elif(type(frame) == frames.DEV_ADV):    # DEV_ADV sqn
                self.dev_sqn = self.dev_sqn + 1
            elif(type(frame) == frames.DEV_ADV):    # LINK_ADV sqn
    ...
    for local in nodes:
        ...
        for link in local.link_tree:
            ...
            for lndev in link.lndev_list:
                ...
                print('\t\t\t\t\t\t\t', "hello_array = ", list(itertools.islice(lndev.rx_probe_record.hello_array, 0, 38)))
                print('\t\t\t\t\t\t\t', list(itertools.islice(lndev.rx_probe_record.hello_array, 38, 81)))
                print('\t\t\t\t\t    ', list(itertools.islice(lndev.rx_probe_record.hello_array, 80, 127)))
...
def packet_received(self, ip6):
    ...
    if(active_known < active_devices):          # if there is a mismatch, then there is new active device
        for dev in dev_list:
            if((dev.type != 0) and (dev.active == 1) and (dev.idx > len(link_list[link_index].lndev_list))):  
                mismatch = 1
                lndev = nodes.link_dev_node(key = nodes.link_dev_key(link_list[link_index], dev), rx_probe_record= nodes.lndev_probe_record(hello_array = deque(127*[0], 127)))
    ...
    elif(main_local.link_tree and ((dev_req == 1) or (mismatch == 1))):
        for dev in dev_list:                                                #### DEV_ADV
            if((dev.type != 0) and (dev.active == 1)):
                mac_converted = int(dev.mac[0]+dev.mac[1]+dev.mac[3]+dev.mac[4]+dev.mac[6]+dev.mac[7]+dev.mac[9]+dev.mac[10]+dev.mac[12]+dev.mac[13]+dev.mac[15]+dev.mac[16], 16)
                fhandler.dev_msgs2send.append(frames.DEV_ADV_msg(
    ...
    if(main_local.link_tree and (outdated_link == 1)):
        ...
        if(local_list[local_index].link_adv_sqn == -1):                     #### LINK_ADV (unsolicited)
            lndev_count = 0
            for lndev in link_list[link_index].lndev_list:
                if(lndev.key.dev.active == 1):
...
def form_frames2send_list(frame_handler):	
    ...
    if frame_handler.link_req_ids:                                          #### LINK_REQ
...
pktSqn = random.randint(0,pktSqnMax)
...
while True:
	...
	packet_header = bmx.packet_header(16,0,0,0,fhandler.link_sqn,pktSqn,main_local.local_id,device_idx)
